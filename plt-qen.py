...
files = glob.glob("/mnt/point/shape/qen/tabla.txt")
...
for i in files:
    ...
    plt.title('QEN - Resolution & Inclination & Mean Distance & # of vectors & Standard Error')



    v1 = plt.scatter([],[],s=min(SE)*1500, marker='x',color='#555555')
    v2 = plt.scatter([],[],s=np.mean(SE)*1500, marker='x',color='#555555')
    v3 = plt.scatter([],[],s=max(SE)*1500, marker='x',color='#555555') #qen

    plt.legend((v1,v2,v3),
       ("{0:.2f}".format(min(SE)*100)+'%', "{0:.2f}".format(np.mean(SE)*100)+'%', "{0:.2f}".format(max(SE)*100)+'%'),
       scatterpoints=1,
       loc='lower left',
       ncol=1,
